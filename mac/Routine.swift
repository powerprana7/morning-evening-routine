...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    func buildMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "루틴 정보", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: "")
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "가리기", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "루틴 종료", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
